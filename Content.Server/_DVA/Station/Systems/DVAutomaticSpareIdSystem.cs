...
public sealed partial class DVAutomaticSpareIdSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<DVAutomaticSpareIdComponent, MapInitEvent>(OnMapInit);
        SubscribeLocalEvent<DVAutomaticSpareIdComponent, PlayerJobAddedEvent>(OnPlayerJobAdded);
        SubscribeLocalEvent<DVAutomaticSpareIdComponent, PlayerJobsRemovedEvent>(OnPlayerJobsRemoved);
    ...
    private void Timeout(Entity<DVAutomaticSpareIdComponent> ent)
    {
    ...
    private void RoundStartNoCaptain(Entity<DVAutomaticSpareIdComponent> ent)
    {
        if (_autoUnlock)
    ...
    private static void RoundStartCaptain(Entity<DVAutomaticSpareIdComponent> ent)
    {
        ent.Comp.State = AutomaticSpareIdState.CaptainPresent;
        ent.Comp.Timeout = null;
    }

    private void OnMapInit(Entity<DVAutomaticSpareIdComponent> ent, ref MapInitEvent args)
    {
        ent.Comp.Timeout = _timing.CurTime + _alertDelay;
    }

    private void OnPlayerJobAdded(Entity<DVAutomaticSpareIdComponent> ent, ref PlayerJobAddedEvent args)
    {
        if (args.JobPrototypeId == ent.Comp.CaptainJob)
            MoveToCaptainPresent(ent);
    }

    private void OnPlayerJobsRemoved(Entity<DVAutomaticSpareIdComponent> ent, ref PlayerJobsRemovedEvent args)
    {
    ...
    private void OnWarDeclared(ref WarDeclaredEvent args)
    {
        ...
        {
            foreach (var spareId in EntityQuery<DVAutomaticSpareIdComponent>())
            {
    ...
    private bool HasCaptain(Entity<DVAutomaticSpareIdComponent> ent)
    {
    ...
    private void MoveToAwaitingUnlock(Entity<DVAutomaticSpareIdComponent> ent)
    {
    ...
    private void MoveToUnlocked(Entity<DVAutomaticSpareIdComponent> ent, ProtoId<AccessLevelPrototype>? newSpareIdAccess = null, LocId? unlockMessageLocId = null)
    {
    ...
    private void MoveToCaptainPresent(Entity<DVAutomaticSpareIdComponent> ent)
    {
    ...
    private void MoveToAlerted(Entity<DVAutomaticSpareIdComponent> ent)
    {
    ...
    /// <param name="ent">The station entity that has the <see cref="DVAutomaticSpareIdComponent"/>.</param>
    /// <param name="newSpareIdAccess">The access to give to the spare ID cabinet. If not specified or null, will default to ent.Comp.GrantAccessToCommand</param>
    private void UnlockCabinet(Entity<DVAutomaticSpareIdComponent> ent, ProtoId<AccessLevelPrototype>? newSpareIdAccess = null)
    {
        var query = EntityQueryEnumerator<DVSpareIDSafeComponent, AccessReaderComponent>();
