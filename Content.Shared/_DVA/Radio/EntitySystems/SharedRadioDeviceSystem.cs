...
using System.Linq;
using Content.Shared._DVA.Radio.Components;
...
public abstract partial class SharedRadioDeviceSystem
{
    ...
    private void SetMicrophoneEnabled(EntityUid uid, EntityUid? user, bool enabled, bool quiet = false, RadioMicrophoneComponent? component = null)
    {
        ...
        if (!quiet && user != null)
        {
            var state = GetStatusText(uid, component);
    ...
    private void OnExamineMicrophone(EntityUid uid, RadioMicrophoneComponent component, ExaminedEvent args)
    {
        ...
        var status = GetStatusText(uid, component, true);

        // Analogous to Encryption Key Holder examine.
        using (args.PushGroup(nameof(RadioMicrophoneComponent)))
        {
            args.PushMarkup(Loc.GetString("radio-microphone-component-on-examine",
                ...
                ("frequency", proto.Frequency / 10f))); // repeating magic numbers across multiple systems :)
        }
    }
    ...
    /// <returns></returns>
    private string GetStatusText(EntityUid uid, DVRadioToggleable component, bool color = false)
    {
    ...
    /// </summary>
    /// <param name="component">Component is a DVRadioToggleabe, so basically RadioMicrophone or RadioSpeaker component</param>
